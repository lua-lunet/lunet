...
PAXE is lunet's datagram encryption extension (authoritative spec:
PAXE.md): one 32-byte shared key per link, addressed by (peer node id,
epoch); AES-256-GCM frames in two modes — standard below 64-byte
payloads, DEK at and above — selected automatically; and per-socket UDP
protection via paxe.protect().
...
Numbered steps:
   ...
   5. paxe.open(frame)                    decrypt: payload + AUTHENTICATED fromId + channel
   6. automatic mode selection            frame sizes differ across the 64-byte boundary
   7. an open() rejection                 the reason lives in the counters, never the return value
   8. epoch retirement                    a rolling key change — no flag day
   9. paxe.protect(sock)                  a datagram genuinely protected on a real UDP socket
  10. keystore_clear() + shutdown()       key erasure, final statistics, the counter invariant
...
local function main()
  ...
  log("ok", "PAXE initialised (AES-256-GCM available; process core dumps disabled "
    .. "so crash dumps cannot carry key material — PAXE.md \"Security Considerations\")")
  ...
  -- a third node does not hold the A<->B key (PAXE.md "One Key Per Link").
  ...
  -- ==========================================================================
  log("info", "Step 5: automatic mode selection — the 64-byte boundary on the wire")
  -- ==========================================================================
  -- The sender selects the frame mode by payload size: standard below 64
  -- bytes, DEK at and above (64 = one CPU cache line; fixed by the
  -- protocol so both ends always agree on the layout).
  local s_tx = paxe.stats()
  local p63 = string.rep("s", 63) -- below 64 -> standard
  local p64 = string.rep("D", 64) -- at 64    -> DEK
  ...
  check(#frame64 == 64 + paxe.OVERHEAD_DEK and dek_flag(frame64) == 1,
    "64-byte payload sealed DEK (DEK flag set, +83 bytes: a wrapped per-frame key rides along)")
  local s_tx2 = paxe.stats()
  check(s_tx2.tx_standard == s_tx.tx_standard + 1 and s_tx2.tx_dek == s_tx.tx_dek + 1,
    "tx counters split by mode: tx_standard +1 (p63), tx_dek +1 (p64)")
  print("  One extra payload byte grew the frame by 47 bytes: the mode switched automatically.")
  ...
  -- rather than an event (PAXE.md "Rotation").
  ...
  -- The mode-boundary frames open too, each reporting its own mode.
  ...
  check(d64 == p64 and m64 == "dek", "64-byte frame opened: byte-exact, mode reported \"dek\"")
  ...
  -- (PAXE.md "Failure Handling"): EVERY frame-level failure — too short,
  ...
  check(s_rej3.rx_auth_fail == s_rej2.rx_auth_fail + 1,
    "rx_auth_fail +1 (a corrupted wrapped DEK surfaces at the payload tag check)")
  ...
  print(string.format("    rejects: plaintext=%d short=%d bad_flags=%d len_mismatch=%d "
    .. "no_peer=%d no_epoch=%d dek_len_mismatch=%d auth_fail=%d",
  ...
  -- The item08 invariant: every frame presented to a configured receiver
  ...
  print("  - automatic standard/DEK mode selection at the 64-byte boundary")
  ...
  print("")
  print("See PAXE.md for the full protocol specification.")
