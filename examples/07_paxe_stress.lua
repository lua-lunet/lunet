...
loudly, not to make a number look good. Under configurable load it:
  ...
  - varies payload sizes across the old 64-byte seam
    (63, 64, 65, 128, 1024 bytes — all sealed standard: the C ABI has no
  ...
  - reconciles the final counters against the operations performed,
    including the counter invariant:
...
-- Payload sizes straddle the old 64-byte seam; every one seals standard
-- (there is no size-based mode selection in this API). Deterministic
-- round-robin: reproducible op mix.
...
local function run_op(i, tally, bad)
  ...
  local want_overhead = paxe.OVERHEAD_STANDARD -- every seal is standard
  local want_mode = "standard"
