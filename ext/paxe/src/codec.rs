...
mod tests {
        ...
        for value in 0u16..=255 {
            ...
            match Flags::from_byte(byte) {
                Ok(flags) => {
                    ...
                    assert_eq!(flags.epoch().bits(), byte >> 3, "epoch wrong for {byte:#04x}");
    ...
    fn header_field_boundaries_round_trip_both_directions() {
        ...
        for h in [
            Header { from_id: 0, to_id: 0, channel: 0, length: 0 },
            Header {
                from_id: u16::MAX,
                to_id: u16::MAX,
                channel: u16::MAX,
    ...
    fn short_inputs_report_too_short_and_extras_are_ignored() {
        ...
        let prefix = serialize_prefix(
            &Header { from_id: 7, to_id: 8, channel: 100, length: 42 },
    ...
    fn seeded_property_no_panic_and_total_mapping() {
        ...
        eprintln!("codec property seed: {seed:#018x} (reproduce with PAXE_CODEC_PROP_SEED={seed:#x})");
        let mut rng = XorShift64::new(seed);

        const ITERATIONS: usize = 1_000_000;
        let mut accepted = 0usize;
        // Explicit degenerate shapes first, then the random stream.
        let degenerates: [[u8; PREFIX_LEN]; 2] =
            [[0x00; PREFIX_LEN], [0xFF; PREFIX_LEN]];
        ...
        assert!(accepted > 100_000, "seed {seed:#x}: accept count {accepted}");
        assert!(accepted < ITERATIONS - 100_000, "seed {seed:#x}: accept count {accepted}");
    }
}
