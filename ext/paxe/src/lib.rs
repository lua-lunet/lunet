...
//!
//! The protocol itself lives in `paxe-core`, pinned by tag in Cargo.toml
//! and published independently. This crate is the lunet-facing boundary
//! and nothing else: it owns the process-global session state (the
//! keystore, the local node identity, the failure policy, the last-error
//! buffer) because key material must never cross into Lua, and it exports
//! the `lunet_paxe_*` symbols `ext/paxe/paxe.lua` declares.
//!
...
//! semantics; the pinned tag plus this crate's test suite keep the two in
//! step.
