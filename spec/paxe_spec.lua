-- Behavioural suite for the lunet.paxe extension (item13): the Lua
-- boundary of the Rust cdylib — argument validation, return shapes,
-- error conventions, key lifecycle, opaque rejection and round trips.
-- The crate internals are item11's and the wire format is item12's;
-- what ONLY this suite can pin is what a script author observes.
--
...
-- no-crash assertion.
--
-- RESET DISCIPLINE (stated, per item13): the module holds
-- process-global state — the keystore, the local node identity, the
-- failure policy, the log-once memo, and cumulative counters that
-- NEVER reset while the process lives. busted runs the whole spec/
-- directory in ONE process, so every test re-establishes its own
-- preconditions in before_each: shutdown() (forgets the identity,
-- zeroes every key, resets the log-once memo) + init() (idempotent)
-- + set_local_id() + the policy pinned to "silent". Tests that need a
-- different identity reconfigure() locally.
--
...
-- intercept a C-level fprintf — and are item20 wiring's job. Full
...
describe("PAXE Module #native", function()
  ...
  -- deleted C implementation (pre-item10) exporting the OLD API;
  -- package.preload is searched before package.cpath, so the
  -- Rust-backed loader always wins. The pending gate still fires
  -- exactly when the cdylib is absent: paxe.lua reads the protocol
  -- constants from the library at load time and raises when the
  ...
  -- message must name its constraint (PAXE.md "Error conventions").
  ...
  describe("module surface", function()
    ...
    it("verifies the overhead constants by MEASUREMENT, not by restated literals", function()
      ...
      assert.are.equal(64 + paxe.OVERHEAD_DEK, #frame64)
      -- The largest legal DEK payload produces exactly max + overhead.
      local frame_max = assert(paxe.seal(string.rep("x", paxe.MAX_PAYLOAD_DEK), NODE_B, CHAN))
      assert.are.equal(paxe.MAX_PAYLOAD_DEK + paxe.OVERHEAD_DEK, #frame_max)
  ...
  describe("operational failures (nil, message — never a raise)", function()
    ...
    it("an oversized payload returns nil, message naming the DEK maximum and moves tx_oversize", function()
      assert(paxe.keystore_set(NODE_B, EPOCH, KEY))
      local frame, err
      local d = deltas(function()
        frame, err = paxe.seal(string.rep("x", paxe.MAX_PAYLOAD_DEK + 1), NODE_B, CHAN)
      end)
      assert.is_nil(frame)
      assert.is_string(err)
      assert.is_true(err:find(tostring(paxe.MAX_PAYLOAD_DEK), 1, true) ~= nil)
  ...
  describe("return shapes", function()
    ...
    it("open yields the same four shapes for a DEK frame", function()
      local payload = string.rep("D", 100)
      local frame = seal_as_a_for_b(payload)
      local plain, from_id, channel, mode = paxe.open(frame)
      assert.are.equal(payload, plain)
      assert.are.equal(NODE_A, from_id)
      assert.are.equal(CHAN, channel)
      assert.are.equal("dek", mode)
    end)
    ...
    it("stats() returns the 14 documented counters, all numbers", function()
      local s = paxe.stats()
  ...
  describe("round trips through the binding", function()
    ...
    it("crosses the 64-byte mode boundary at 63, 64 and 65 bytes", function()
      for _, size in ipairs({ 63, 64, 65 }) do
        ...
        if size < 64 then
          ...
          assert.are.equal(0, frame:byte(9) % 2) -- DEK flag clear
        else
          assert.are.equal("dek", mode)
          assert.are.equal(size + paxe.OVERHEAD_DEK, #frame)
          assert.are.equal(1, frame:byte(9) % 2) -- DEK flag set
        end
      end
    end)
    ...
    it("counts sealed frames by mode (tx_total = tx_standard + tx_dek)", function()
      assert(paxe.keystore_set(NODE_B, EPOCH, KEY))
      local d = deltas(function()
        assert(paxe.seal("small", NODE_B, CHAN))              -- standard
        assert(paxe.seal(string.rep("x", 64), NODE_B, CHAN))  -- DEK
      end)
      assert.are.equal(2, d.tx_total)
      assert.are.equal(1, d.tx_standard)
      assert.are.equal(1, d.tx_dek)
    end)
  end)
  ...
  describe("opaque rejection (no decryption oracle)", function()
    ...
    it("reveals the rejection reason ONLY through counter deltas", function()
      ...
      -- A 37-byte frame carrying the DEK bit: under the 83-byte DEK
      -- minimum, counted as short (the parse-geometry gate).
      assert_one_reason(string.rep("\0", 8) .. string.char(0x1D) .. string.rep("\0", 28),
        "rx_short")
      -- A DEK frame whose redundant inner Length (bytes 65-66) is
  ...
  describe("failure policy selection", function()
    ...
    -- item20 subprocess wiring. Only the selection API is pinned.
  ...
  describe("protect/unprotect/is_protected", function()
    ...
    -- end-to-end coverage is item14's. What IS pinned here: every
