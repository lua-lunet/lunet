-- item14 PERMANENT end-to-end test, RECEIVER side (PAXE node 200).
-- Driven by test/run_paxe_udp_e2e.sh together with test/paxe_udp_sender.lua
-- (node 100) — two processes because the Rust keystore holds ONE node
-- identity per process. Evolves item09's ad-hoc verification into the
-- permanent wire test that proves PAXE protects REAL UDP traffic, kept
-- runnable as a smoke/CI check (exit 0 only when every check passes).
--
...
--   4. MODE BOUNDARY over the wire: 63, 64 and 65-byte payloads
--      (standard / DEK / DEK) all traverse a real socket.
--   5. MAX PAYLOAD: 65424 bytes (the documented DEK maximum) — the frame
--      is 65507 bytes, exactly the UDP datagram limit (65535 - 20 - 8).
...
--
-- TIMEOUT INTERPRETATION (item14): every receive is bounded by the
...
local function main()
  print("=== paxe udp e2e (item14): receiver (node 200) ===")
  ...
  lunet.spawn(function()
    ...
    check(s1.rx_short == s0.rx_short and s1.rx_len_mismatch == s0.rx_len_mismatch
      ...
      and s1.rx_dek_len_mismatch == s0.rx_dek_len_mismatch and s1.rx_auth_fail == s0.rx_auth_fail,
  ...
  if not expect_frame(P64, "64-byte payload (DEK)") then return finalize(false) end
  if not expect_frame(P65, "65-byte payload (DEK)") then return finalize(false) end
  if not expect_frame(PLARGE, "8192-byte payload (DEK, large frame)") then return finalize(false) end
  ...
  if #ctl == RAW_CONTROL_LEN then
    ...
    max_wire = true
    if not expect_frame(PMAX, "max payload 65424 bytes (frame 65507 = the UDP datagram limit)") then
      return finalize(false)
    end
  else
