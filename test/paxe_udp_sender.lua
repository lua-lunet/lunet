-- PERMANENT end-to-end test, SENDER side (PAXE node 100). Driven by
...
-- tx_standard is what ties each payload to its on-the-wire frame
-- geometry — the C ABI seals standard frames at every payload size).
--
...
-- 63/64/65/8192/max bytes (all standard — the max frame is 65507 bytes,
-- exactly the UDP datagram limit); a raw 65507-byte control plus marker
-- on an UNPROTECTED socket so the receiver can tell a platform that
-- cannot carry a maximum datagram (macOS loopback: EMSGSIZE) apart from
-- a genuine PAXE drop; then the concurrency burst — 4 coroutines, each
-- on its own protected socket, 16 distinguishable payloads interleaved
-- with sleep(0).
--
...
local function main()
  ...
  -- standard-mode maximum; nothing is sealed or transmitted.
  local s0 = paxe.stats()
  local ok, err = udp.send(sockA, "127.0.0.1", PORT_MAIN, string.rep("x", paxe.MAX_PAYLOAD_STANDARD + 1))
  check(ok == nil and type(err) == "string" and err:find("65470", 1, true) ~= nil,
    "oversized send fails with a clear error naming 65470")
  local s1 = paxe.stats()
  check(s1.tx_oversize == s0.tx_oversize + 1, "tx_oversize +1")
  check(s1.tx_total == s0.tx_total, "an oversized offer seals nothing")

  -- Positive sequence: 63/64/65/8192/max, ALL standard (the C ABI has no
  -- size-based mode selection). The max frame is 65470 + 37 = 65507 bytes
  -- = exactly the UDP datagram limit: a sizing error in the documented
  -- maximum fails the seal HERE, and a synchronous transport refusal
  -- fails the send HERE.
  ...
  check(udp.send(sockA, "127.0.0.1", PORT_MAIN, PMAX) == true,
    "send max payload 65470 bytes (frame 65507 = the UDP datagram limit)")
  local s2 = paxe.stats()
  check(s2.tx_total == s1.tx_total + 5, "tx_total +5 for the positive sequence")
  check(s2.tx_standard == s1.tx_standard + 5,
    "all five positive payloads sealed standard (no size-based mode selection)")
  check(s2.tx_dek == s1.tx_dek, "tx_dek unchanged (the C ABI exposes no fanout sealer)")
  ...
  check(s3.tx_standard == s2.tx_standard + CONC_N * CONC_K,
    "concurrency payloads all sealed standard")
