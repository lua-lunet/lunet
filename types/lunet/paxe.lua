...
---@field tx_dek integer Reusable-DEK fanout frames recorded by a Rust host (the C ABI never seals DEK)
---@field tx_oversize integer Seals refused: payload over the standard-mode maximum
...
---Reusable-DEK fanout frame overhead in bytes. The C ABI exposes no fanout
---sealer; this describes DEK frames this host may receive from a Rust host.
---@type integer
paxe.OVERHEAD_DEK = nil

---Largest payload sealable in standard mode (the only mode `seal` produces).
---@type integer
paxe.MAX_PAYLOAD_STANDARD = nil

---Largest payload in a reusable-DEK fanout frame this host may receive.
---@type integer
...
---Seal `payload` for `to_id` on `channel` as a standard one-recipient frame
---(the C ABI's only sealing mode; reusable-DEK fanout sealing is Rust
---API-only). The frame's fromId is the configured local id; the send epoch is
---the newest epoch installed for `to_id`.
